...
from  register.models import facultysignup
from  register.models import student
from  register.models import studentattend
from  register.models import facleave
from  register.models import studleave
from  register.models import studentmark
...
def facultysign(request):
    if request.method== 'POST':
        ...
        c=facultysignup(factid=faid,name=name,address=address,dob=dob,gender=gender,qualification=qualification,mobile=mobile,
...
def facultyleave(request):
    if request.method=='POST':
        ...
        f=facleave(name=name,reason=reason,fromdate=fromdate,todate=todate)
        f.save()
    return render(request,'facapplyleave.html')
def studentleave(request):
    if request.method=='POST':
        ...
        fromdate=request.POST.get('fromdate')
        todate=request.POST.get('todate')
        g=studleave(name=name,reason=reason,fromdate=fromdate,todate=todate)
        g.save()
